...
let expName = 'kirjaimet10b';  // from the Builder filename that created this script
...
function experimentInit() {
  ...
  v16_oikea = new visual.ImageStim({
    win : psychoJS.window,
    name : 'v16_oikea', units : undefined, 
...
function trialRoutineBegin() {
  ...
  trialComponents.push(v16_oikea);
...
function trialRoutineEachFrame() {
  ...
  if (mouse.status === PsychoJS.Status.STARTED) {  // only update if started and not finished!
    ...
    if (!buttons.every( (e,i,) => (e == prevButtonState[i]) )) { // button state changed?
      ...
      if (buttons.reduce( (e, acc) => (e+acc) ) > 0) { // state changed to a new click
        ...
        gotValidClick = false;
        for (const obj of [v04,v05,v08,v09,v10,v14,v15,v16_oikea,v19,v20]) {
  ...
  // *v16_oikea* updates
  if (t >= 0.5 && v16_oikea.status === PsychoJS.Status.NOT_STARTED) {
    // keep track of start time/frame for later
    v16_oikea.tStart = t;  // (not accounting for frame time here)
